...
public class RJ_CAKU {
	...
	public static void main(String[] args) {
		...
		RJ_CAKU caku = new RJ_CAKU(dataTrace, tempMisclassificationCost, tempTeachCost);
...
//			System.out.println(
//					"(querySplitClassify) The block origin instancestates  is : " + instanceStatus[paraBlock[i]]);
//		} //of if
		...
		// Check the first instance states 
...
//		for (int i = 0; i < paraBlock.length; i++) {
//			System.out.println("(querySplitClassify)  The block instancestates after query the initial point is : "
//					+ instanceStatus[paraBlock[i]]);
//		} //of if
		...
		for (int i = 0; i < paraBlock.length; i++) {
			...
			if (instanceStatus[paraBlock[i]] == 1) {
				System.out.println("(querySplitClassify) The state of paraBlcok[" + paraBlock[i] + "] is : "
						+ instanceStatus[paraBlock[i]]);
				tempQueriedNum++;
			} // of if
		} // of for i
		...
		// need to modify
		int tempNumToBuy = numToBuy(paraBlock, labels[paraInitialPoint]);
		...
		int tempFirstLabel = (int) (data.instance(paraInitialPoint).classValue());
		//		if (tempQueriedNum > 1) {
		//			for (int i = 1; i < tempQueriedNum; i++) {
		//				if ((int) (data.instance(tempQueried[i]).classValue()) != tempFirstLabel) {
		//					tempPure = false;
		//					break;
		//				} // of if
		//			} // of for i
		...
		// Step 4. Query other instances one by one and update the second center
		int tempSecondCenter = -1;
		System.out.println("(querySplitClassify) Enter the buy one by one");
		for (int i = 0; i < tempNumToBuy; i++) {
			System.out.println("(querySplitClassify) Let me see the fathest detail..............");
			...
			System.out.println("(querySplitClassify) Am i in the querySplitClassify ");
		...
		if (!tempPure) {
			...
			tempCenterIndex[0] = paraInitialPoint;
			...
			System.out.println("(querySplitClassify) The cluster result  is : " + Arrays.deepToString(tempSplitted));

			int[] totalNumOfSplittedArray = new int[tempSplitted.length];
			Arrays.fill(totalNumOfSplittedArray, 0);
			for (int i = 0; i < tempSplitted.length; i++) {
				for (int j = 0; j < tempSplitted[i].length; j++) {
					if (tempSplitted[i][j] != -1) {
						totalNumOfSplittedArray[i]++;
					} // of if
				} // of for j
			} // of for i

			System.out.println(
					"(querySplitClassify) The length of array is " + intArrayToString(totalNumOfSplittedArray));
			int[] Splitted1 = new int[totalNumOfSplittedArray[0]];
			int[] Splitted2 = new int[totalNumOfSplittedArray[1]];
			for (int i = 0; i < totalNumOfSplittedArray[0]; i++) {
				Splitted1[i] = tempSplitted[0][i];
			} // of for i
			for (int i = 0; i < totalNumOfSplittedArray[1]; i++) {
				Splitted2[i] = tempSplitted[1][i];
			} // of for i

			System.out.println("(querySplitClassify) The splitted1 is  " + intArrayToString(Splitted1));
			System.out.println("(querySplitClassify) The splitted2 is  " + intArrayToString(Splitted2));
			querySplitClassify(Splitted1, Splitted1[0]);
			System.out.println("(querySplitClassify) After oeratation cluster #1.... ");
			querySplitClassify(Splitted2, Splitted2[0]);
			System.out.println("(querySplitClassify) The Splitted2 initialPoint is: " + Splitted2[0]);
			System.out.println("(querySplitClassify) After oeratation cluster #2.... ");
	...
	public int[][] cluster(int[] paraBlock, double[][] paraCenters) {
		...
		// Step 1. Initialize
		int paraK = 2;
		int[][] tempCluster = new int[paraK][data.numInstances()];
		int[][] resultCluster = new int[paraK][paraBlock.length];
		...
		while (!doubleMatricesEqual(tempCenters, tempNewCenters)) {
			...
			// Step 2.1 Cluster
			for (int i = 0; i < paraBlock.length; i++) {
				if (distance(paraBlock[i], tempCenters[1]) < distance(paraBlock[i], tempCenters[0])) {
				...
				System.out.println("The parablock[" + i + "] is:  " + paraBlock[i]);
			} // Of for i
			
			System.out.println("All the block number: "+ paraBlock.length + "\rThe 1 cluster number is " + tempKindSize[0] + "\rThe 2 cluster number is " + tempKindSize[1]);
			 System.out.println("Current cluster: " + Arrays.deepToString(tempCluster));
...
//				tempCounters[tempCluster[paraBlock[i]]]++; // nice expect the center
				for (int j = 0; j < data.numAttributes() - 1; j++) {
					tempNewCenters[i][j] += data.instance(paraBlock[i]).value(j); // include the center
				} // Of for j
			} // Of for i
		...
		// System.out.println(intArrayToString(tempCluster));
//		int[] tempCollumIndex = new int[paraK];
//		// System.out.println(intArrayToString(tempCollumIndex));
//		Arrays.fill(tempCollumIndex, 0);
//		// It's OK, but has some zero occupy the position
//		for (int i = 0; i < paraK; i++) {
//			for (int j = 0; j < data.numInstances(); j++) {
//				if (tempCluster[paraBlock[j]] == i) {
//					resultCluster[i][tempCollumIndex[i]++] = j;
//				}
//			}
//		} // Of for i
		System.out.println("(cluster)The result cluster is " + Arrays.deepToString(tempCluster));
		return tempCluster;
	}// Of cluster
	...
	public int findFarthest(int[] paraBlock, int[] paraLabeledInstances) {
		...
		for (int i = 0; i < paraBlock.length; i++) {
			...
			for (int j = 0; j < paraLabeledInstances.length; j++) {
				...
				//System.out.println("(findFarthest)" + paraBlock[i] + " to labeled  " + paraLabeledInstances[j] + "  = " + manhattanDistance(paraBlock[i], paraLabeledInstances[j]));
			} // Of for j
			...
			//System.out.println("(findFarthest) The Max distance is " + tempMaxDistanceSum + " and the index is  " + resultFarthest);
	...
	public static void testTotalCost() {
		...
		double[] tempMisclassificationCost = { 2, 4 };
		double tempTeachCost = 1;
		RJ_CAKU caku = new RJ_CAKU("src/data/rj4instancetest.arff", tempMisclassificationCost, tempTeachCost);
	...
	public double[][] indexToInstance(int[] paraIndex) {
		//		System.out.println("(indexToInstance)The data attributes number is : " + data.numAttributes());
		//		System.out.println("(indexToInstance)The paraIndex is: " + intArrayToString(paraIndex));
	...
	public static void testIndexToInstance() {
		...
		double[] tempMisclassificationCost = { 2, 4 };
		double tempTeachCost = 1;
		RJ_CAKU caku = new RJ_CAKU("data/rj4instancetest.arff", tempMisclassificationCost, tempTeachCost);
