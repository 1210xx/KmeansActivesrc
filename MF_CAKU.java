...
public class MF_CAKU {
	 ...
	 *********************
	 */
	public MF_CAKU(String paraFilename, double[] paraMisclassificationCosts,
			double paraTeacherCost) {
		...
		File outputFile = new File("data/mfresults.txt");
	...
	public double computeTotalCost() {
		...
		for (int i = 0; i < instanceStatus.length; i++) {
			if (instanceStatus[i] == UNTREATED) {
				System.out.println("Algorithm error! Instance " + i
				...
				if ((labels[i] == 0)
						&& (int) data.instance(i).classValue() == 1) {
				...
				System.out.println("System error! Instance " + i
						+ " has an unrecognized status: " + instanceStatus[i]);
				System.exit(0);
			}// Of if
		}// Of for i
	...
	double euclideanDistance(int paraInstanceIndex, double[] paraPoint) {
		...
		for (int i = 0; i < paraPoint.length; i++) {
			tempDifference = data.instance(paraInstanceIndex).value(i)
					- paraPoint[i];
	...
	double euclideanDistance(int paraFirstIndex, int paraSecondIndex) {
		...
		for (int i = 0; i < data.numAttributes() - 1; i++) {
			tempDifference = data.instance(paraFirstIndex).value(i)
					- data.instance(paraSecondIndex).value(i);
			tempDistance += tempDifference * tempDifference;
		}// Of for i
	...
	int[][] twoMeansClustering(int[] paraBlock, int[] paraInitalCenterIndices) {
		...
		for (int i = 0; i < 2; i++) {
			for (int j = 0; j < data.numAttributes() - 1; j++) {
				tempNewCenters[i][j] = data
						.instance(paraInitalCenterIndices[i]).value(j);
			}// Of for j
		}// Of for i
		...
		while (!tempConverged) {
			...
			for (int i = 0; i < paraBlock.length; i++) {
				if (euclideanDistance(paraBlock[i], tempNewCenters[0]) < euclideanDistance(
						paraBlock[i], tempNewCenters[1])) {
			...
			for (int i = 0; i < 2; i++) {
				...
				for (int j = 0; j < tempSizes[i]; j++) {
					for (int k = 0; k < data.numAttributes() - 1; k++) {
						tempNewCenters[i][k] += data.instance(tempBlocks[i][j])
								.value(k);
					}// Of for k
				}// Of for j
		...
		if (tempNewBlocks[0].length == 0) {
			for (int i = 0; i < tempNewBlocks[1].length; i++) {
				System.out.println(data.instance(tempNewBlocks[1][i]));
			}//Of for i
		}//Of if

		if (tempNewBlocks[1].length == 0) {
			for (int i = 0; i < tempNewBlocks[0].length; i++) {
				System.out.println(data.instance(tempNewBlocks[0][i]));
			}//Of for i
		}//Of if
	 ...
	 *************************************
	 */
	public static boolean doubleMatricesEqual(double[][] paraFirstMatrix,
			double[][] paraSecondMatrix) {
		// Same number of rows?
	...
	public int findFarthest(int[] paraBlock, int[] paraLabeledInstances,
		...
		for (int i = 0; i < paraBlock.length; i++) {
			...
			for (int j = 0; j < paraActualQueried; j++) {
				...
				tempDistanceSum += euclideanDistance(paraBlock[i],
		...
		//System.out.println("��Զ����������±꣺"+resultFarthest);
	 ...
	 ************************************
	 */
	public void querySplitClassifyRescan(int[] paraBlock) {
		//System.out.println("now learning: " + Arrays.toString(paraBlock));
		String tempMessage;
		tempMessage = "Handle a block with " + paraBlock.length
				+ " instances, ";
		writer.println(tempMessage);
		System.out.println(tempMessage);
		
		if (paraBlock.length == 5) {
			tempMessage = "Instances in this block: ";
			for (int i = 0; i < paraBlock.length; i++) {
				tempMessage += "" + paraBlock[i] + ",";
			}//Of for i
			writer.print(tempMessage + "\r\n");
		}//Of if

		if (paraBlock.length < 3) {
			for (int i = 0; i < paraBlock.length; i++) {
				labels[paraBlock[i]] = (int) data.instance(
						paraBlock[i]).classValue();
				instanceStatus[paraBlock[i]] = QUERIED;
			}//Of for i
			return;
		}//Of if
		...
		int tempFirstLabeledIndex = -1;
		int tempDifferentLabeledIndex = -1;
		...
		// Used to split. Two sub-blocks.
		int[][] tempSubBlocks;
		
		//Step 0. Scan to obtain the first labeled instance.
		for (int i = 0; i < paraBlock.length; i++) {
			if (instanceStatus[paraBlock[i]]  == QUERIED) {
				tempFirstLabel = labels[paraBlock[i]];
				tempFirstLabeledIndex = paraBlock[i];
				tempMessage = "First label given by " + paraBlock[i] + "\r\n";
				writer.print(tempMessage);
				break;
			}//Of if
		}//Of for i
		...
		if (tempFirstLabel == -1) {
			labels[paraBlock[0]] = (int) data.instance(
					paraBlock[0]).classValue();
			instanceStatus[paraBlock[0]] = QUERIED;
			tempFirstLabel = labels[paraBlock[0]];
			tempFirstLabeledIndex = paraBlock[0];
			tempMessage = "First label given by " + paraBlock[0] + "\r\n";
			writer.print(tempMessage);
		}//Of if
		...
		for (int i = 0; i < paraBlock.length; i++) {
			if (paraBlock[i] == paraInitialPoint) {
				...
				if (instanceStatus[paraInitialPoint] != QUERIED) {
					...
					labels[paraInitialPoint] = (int) data.instance(
							paraInitialPoint).classValue();
					tempMessage = "Q1: " + paraInitialPoint + ", ";
					writer.print(tempMessage);
				}// Of if
				...
				tempMessage = "First label given by " + paraInitialPoint + "\r\n";
				writer.print(tempMessage);
		...
		*/
		...
		for (int i = 0; i < paraBlock.length; i++) {
			if (instanceStatus[paraBlock[i]] == QUERIED) {
				tempQueriedInstances[tempNumQueried] = paraBlock[i];
				tempNumQueried++;

				// It is the first labeled instance (with lowest index in the block).
				if (paraBlock[i] == tempFirstLabeledIndex) {
					continue;
				}// Of if
				...
				if (labels[paraBlock[i]] != tempFirstLabel) {
					tempPure = false;
					tempDifferentLabeledIndex = paraBlock[i]; 
					break;
				}// Of if
			} // Of if
		} // Of for i
		//System.out.println("" + tempNumQueried
		//		+ " instances have been queried in this block.");
		//System.out.println("Are they pure? " + tempPure);

		// Step 3. Split if impure.
		int[] tempBrothers = new int[2];
		tempBrothers[0] = tempFirstLabeledIndex;
		if (!tempPure) {
			tempBrothers[1] = tempDifferentLabeledIndex;
			
			//System.out.println("The brothes are: " + Arrays.toString(tempBrothers));
			tempSubBlocks = twoMeansClustering(paraBlock, tempBrothers);

			//System.out.println("Split in two: " + tempSubBlocks[0].length
			//		+ " and " + tempSubBlocks[1].length);
			tempMessage = "First elements of blocks: " + tempSubBlocks[0][0]
					+ ", " + tempSubBlocks[1][0] + "\r\n";
			writer.print(tempMessage);

			querySplitClassifyRescan(tempSubBlocks[0]);
			querySplitClassifyRescan(tempSubBlocks[1]);
			//���ﲻһ�����ҵĲ�������brother�ȷֿ���Ȼ�����Ҳ�ͬ��Ȼ������ҵ�һ���㣬������Ƿֿ�������������ǩ��ͬ�ĵ�Ϊ
			//��һ���㣬�ٴβ�֣��������ﲽ����в�ͬ������
			
			return;
		}// Of if

		// Step 4. How many labels should be queried?
		int[] tempLookups = lookup(paraBlock.length);
		//int tempNeededQueries = (int)Math.sqrt(paraBlock.length)
		int tempNeededQueries = tempLookups[tempFirstLabel]
				- tempNumQueried;
		tempMessage = "tempNumQueried = " + tempNumQueried + ", tempNeededQueries = " + tempNeededQueries;
		writer.println(tempMessage);

		System.out.println(tempMessage);

		// Step 5. Query enough labels.
		int tempNextToQuery;
		for (int i = 0; i < tempNeededQueries; i++) {
			// Find the next to query.
			tempNextToQuery = findFarthest(paraBlock, tempQueriedInstances,
					tempNumQueried);

			// Query
			tempQueriedInstances[tempNumQueried] = tempNextToQuery;
			instanceStatus[tempNextToQuery] = QUERIED;
			labels[tempNextToQuery] = (int) data.instance(tempNextToQuery)
					.classValue();
			tempMessage = "Q3: " + tempNextToQuery + ", ";
			writer.print(tempMessage);
			tempNumQueried++;

			// Split if impure
			//System.out.print(", " + tempNextToQuery + "("
			//		+ labels[tempNextToQuery] + ")");
			if (labels[tempNextToQuery] != tempFirstLabel) {
				//System.out.println("#" + tempNextToQuery
				//		+ " has a different label: " + labels[tempNextToQuery]);
				tempBrothers[1] = tempNextToQuery;
				tempSubBlocks = twoMeansClustering(paraBlock, tempBrothers);

				tempMessage = "Split in two: " + tempSubBlocks[0].length
						+ "(" + tempBrothers[0] + ")"
						+ " and " + tempSubBlocks[1].length
						+ "(" + tempBrothers[1] + ")";
				writer.println(tempMessage);
		...
		//System.out
		//		.println("This block is pure. Classify all others instances as "
		//				+ tempFirstLabel);
		for (int i = 0; i < paraBlock.length; i++) {
	...
	public void querySplitClassify(int[] paraBlock, int paraInitialPoint) {
		...
		String tempMessage;
		tempMessage = "Handle a block with " + paraBlock.length
				+ " instances with initial point " + paraInitialPoint + ", ";
		writer.println(tempMessage);
		System.out.println(tempMessage);
		...
		for (int i = 0; i < paraBlock.length; i++) {
			if (instanceStatus[paraBlock[i]] == QUERIED) {
				...
				// Use the first known label if unspecified by the parameter
				// paraInitialPoint.
		...
		if (tempNumQueried == 0) {
			labels[paraBlock[0]] = (int) data.instance(paraBlock[0])
					.classValue();
			tempMessage = "Q2: " + paraBlock[0] + ", ";
			writer.print(tempMessage);
			instanceStatus[paraBlock[0]] = QUERIED;
	...
	public static String doubleMatrixToString(double[][] paraMatrix,
			int paraDigits, char paraSeparator) {
		String resultString = "[]";
		if ((paraMatrix == null) || (paraMatrix.length < 1))
			return resultString;
		resultString = "[";
		...
		for (int i = 0; i < paraMatrix.length; i++) {
			...
			for (int j = 0; j < paraMatrix[i].length - 1; j++) {
				resultString += ""
						+ ((int) (paraMatrix[i][j] * tempValue) + 0.0)
						/ tempValue + paraSeparator + " ";
			}// Of for j
	...
	private int[] lookup(int paraBlockSize) {
		...
		double[] tmpMinCost = new double[] {
				0.5 * misclassificationCosts[0] * paraBlockSize,
		...
		for (int i = 1; i <= paraBlockSize; i++) {
			...
			for (int j = 0; j < 2; j++) {
				tmpCost[j] = (1 - ra[i]) * misclassificationCosts[j]
						* paraBlockSize + teacherCost * i;
			...
			if (isFind[0] && isFind[1]) {
				...
				for (int j = 0; j <= i; j++) {
					for (int k = 0; k < 2; k++) {
						tmpCost[k] = (1 - ra[j]) * misclassificationCosts[k]
								* paraBlockSize + teacherCost * j;
	...
	public static void main(String[] args) {
		...
		MF_CAKU caku = new MF_CAKU("data/spambase.arff",
		...
		caku.querySplitClassifyRescan(tempInitialBlock);
