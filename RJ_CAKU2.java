...
public class RJ_CAKU2 {
	...
	/**
	 ************************************** 
	 * The constructor.read the file and
	 ...
	 **************************************
	 */
	public RJ_CAKU2(String paraFilename, double[] paraMisclassificationCost, double paraTeachCost) {
		data = null;
		try {
	...
	public static void main(String[] args) {
		...
		double tempCost = 0;
		RJ_CAKU2 caku = new RJ_CAKU2("Data/CAKU.arff", tempMisclassificationCost, tempTeachCost);
...
//		 caku.cluster(caku.prelearn(), caku.indexToInstance(tempCneterIndex));
	...
	public void querySplitClassify(int[] paraBlock, int paraInitialPoint) {
		...
		for (int i = 0; i < paraBlock.length; i++) {
			System.out.println("(querySplitClassify) The block origin instancestates  is : " + instanceStatus[paraBlock[i]]);
		}//of if
		...
		for (int i = 0; i < paraBlock.length; i++) {
			System.out.println("(querySplitClassify)  The block instancestates after query the initial point is : " + instanceStatus[paraBlock[i]]);
		}//of if
		...
		for (int i = 0; i < paraBlock.length; i++) {
			...
			if (instanceStatus[paraBlock[i]] == 1) {
				System.out.println("(querySplitClassify) The state of paraBlcok[" + paraBlock[i] +"] is : " + instanceStatus[paraBlock[i]]);
				tempQueriedNum++;
			} // of if
		} // of for i
...
//		} // of if
		...
		for (int i = 0; i < tempNumToBuy; i++) {
			...
			System.out.println("(querySplitClassify) The tempQueried array after update is : " + intArrayToString(tempQueried));
		...
		if (!tempPure) {
			...
			System.out.println("(querySplitClassify) tempSecondCenter is " + tempSecondCenter);
			...
			int[][] tempSplitted = cluster(paraBlock, tempCenter);
			...
			for (int i = 0; i < tempSplitted.length; i++) {
				for (int j = 0; j < tempSplitted[0].length; j++) {
	...
	public int[][] cluster(int[] paraBlock, double[][] paraCenters) {
		...
		while (!doubleMatricesEqual(tempCenters, tempNewCenters)) {
			...
			// Compute new centers count the number of instances in
			// different class
	...
	public static void testTotalCost() {
		...
		double[] tempMisclassificationCost = { 2, 4 };
		double tempTeachCost = 1;
		RJ_CAKU2 caku = new RJ_CAKU2("src/data/rj4instancetest.arff", tempMisclassificationCost, tempTeachCost);
...
//		System.out.println("(indexToInstance)The paraIndex is: " + intArrayToString(paraIndex));
	...
	public static void testIndexToInstance() {
		...
		double[] tempMisclassificationCost = { 2, 4 };
		double tempTeachCost = 1;
		RJ_CAKU2 caku = new RJ_CAKU2("data/rj4instancetest.arff", tempMisclassificationCost, tempTeachCost);
