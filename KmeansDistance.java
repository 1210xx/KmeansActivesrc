...
public class KmeansDistance {
	...
	public static void main(String args[]) {
		...
		KmeansDistance tempLeaner = new KmeansDistance(
				"src/data/ceshiwenjian.arff", mCost, 1);
	 ...
	 **************************************
	 */
	public KmeansDistance(String paraFilename, double misClassifiedCost[],
			int teachCost) {
	 ...
	 * 
	 * @param paraBlock
	 *            The block of instance
	 **************************************
	 */
	void learning(int[] paraBlock) {
		System.out
				.println("Now learn these data:" + Arrays.toString(paraBlock));
		...
		//Step 2. Find the number of need to buy.
		...
		if (indexOfFirstLabel != -1) {
		 ...
		 //Step 3. Now find other farthest index in this block.
		while (pointIndex.size() <= (needToBuyNum - 1)) {
			...
			if (labels[paraBlock[zuiyuandian]] != labels[paraBlock[pointIndex
					.get(0)]]) {
	 ...
	 *            :paraBlock's First index of instance which has label
	 * @param differentLabelIndex
	 *            :The index of instance which has different label with first
	 *            instance
	 **************************************
	 */
	public void splitAndLearn(int[] paraBlock, int firstlabelIndex,
			int differentLabelIndex) {
		// Step 1. Split
		int[] tempClutering = cluster(firstlabelIndex, differentLabelIndex,
				paraBlock);
	 ...
	 **************************************
	 */
	public int[] cluster(int firstlabelIndex, int differentLabelIndex,
			int[] paraBlock) {
		// Step 1. Initialize
		...
		for (int j = 0; j < data.numAttributes() - 1; j++) {
			tempNewCenters[0][j] = data.instance(paraBlock[firstlabelIndex])
					.value(j);
			tempNewCenters[1][j] = data
					.instance(paraBlock[differentLabelIndex]).value(j);
		}
	 ...
	 * 
	 * @param point
	 *            ��the index of all instance
	 * @param paraBlockPoint
	 *            :the index of all instance
	...
	double computeDistance(int point, int paraBlockPoint) {
		...
		for (int i = 0; i < data.numAttributes() - 1; i++) {
			tempDistance += Math.pow(data.instance(point).value(i)
					- data.instance(paraBlockPoint).value(i), 2);
		} // Of for i
		return tempDistance;
	}// Of computeDistance
	 ...
	 *            :current data set
	 * @param centerPoints
	 *            : the index of farthest centers in now paraBlock
	...
	public int computeFathestPoint(int[] paraBlock, List<Integer> centerPoints) {
		...
		for (int i = 0; i < blockSize; i++) {
			...
			for (int j = 0; j < listSize; j++) {
				tempDistance += computeDistance(paraBlock[centerPoints.get(j)],
						paraBlock[i]);
			} // Of for j
	...
	public double distance(int paraIndex, double[] paraArray) {
		...
		for (int i = 0; i < paraArray.length; i++) {
			resultDistance += Math.pow(data.instance(paraIndex).value(i)
					- paraArray[i], 2);
		} // Of for i
